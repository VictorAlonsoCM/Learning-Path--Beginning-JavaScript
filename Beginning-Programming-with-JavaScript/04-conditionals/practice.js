...
var getTagName = function (tag) {
	var open = tag.slice(tag.indexOf("<")+1, tag.indexOf(">"));
	var close = tag.slice(tag.lastIndexOf("/")+1, tag.lastIndexOf(">"));
	if(open !== close){
...
var improveTweet = function (tweet) {
	return tweet + " lol" + " omg" + " lmao" + " rofl";
};
...
var magic8Ball = function (question) {
	...
	if((typeof question) === "string" && question.indexOf("?") > -1){
		if(answer === 1){
		return "Yes";
		}else if(answer === 2){
			return "No";
...
var interjectAt = function (add, number, string) {

	if(number > string.length){
		throw "the string doesn't have that many letters!"
	}

	if(typeof add === "string" && typeof number === "number"){

		var strStart = string.slice(0, number);
		var add = "-"+add+"-";
		var strEnd = string.slice(number, string.length);
		string = strStart + add + strEnd;
		return string;
	}else{
...
var interjectAt2 = function (add, number, string) {
...
var randomInterject = function (msg) {
	var random = Math.floor((Math.random() * msg.length)+1);
	console.log(random);
	var random2 = Math.floor((Math.random() * 2)+1);
	if(typeof msg === "string"){
		if(random2 === 1){
			return interjectAt2("lol", random, msg);
		}else{
			return interjectAt2("omg", random, msg);
		}
	}else{
